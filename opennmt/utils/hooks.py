...
class LoadCheckpointHook(tf.train.SessionRunHook):
  ...
  def begin(self):
    ...
    for name, shape in var_list:
      if not name.startswith("global_step") and not name.startswith("words_per_sec") and not name.startswith("optim"):
  ...
  def after_create_session(self, session, coord):
    for p, assign_op, (name, value) in zip(self.placeholders, self.assign_ops, six.iteritems(self.values)):
