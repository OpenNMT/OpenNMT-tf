...
class BeamSearch(DecodingStrategy):
  ...
  def step(self, step, sampler, log_probs, cum_log_probs, finished, state, extra_vars):
    ...
    if self.length_penalty != 0:
      expand_sequence_lengths = tf.tile(tf.expand_dims(sequence_lengths, 1), multiples=[1, vocab_size])
      expand_sequence_lengths = tf.reshape(expand_sequence_lengths, [-1, self.beam_size * vocab_size])
      scores /= tf.pow(((5. + tf.cast(expand_sequence_lengths + 1, tf.float32)) / 6.), self.length_penalty)
